...
class test{
    public static void main(String args[]){
        ...
        for(int i=0;i<p;i++){
            res.add(find(par[i][0], par[i][1], emp));
        }
        for(Integer i:res)System.out.print(i+" ");
    }
    public static int find(int idx, int dest, int[] emp){
        int i=idx, j=idx;
        while(i>=0 || j<emp.length){
            if((i>=0 && emp[i]==dest) && (j<emp.length&&emp[j]==dest)){
                return Math.min(idx-i, j-idx);
            }
            else if(i>=0 && emp[i]==dest)return idx-i;
            else if(j<emp.length && emp[j]==dest)return j-idx;
            i--;j++;
        }
        return -1;
    }
