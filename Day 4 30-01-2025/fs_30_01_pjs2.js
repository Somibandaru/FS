...
const remove = employees.findIndex(emp => emp.name === "Emily");
if(remove !== -1){
    employees.splice(remove,1);
}
const lowperf = employees.filter(emp => emp.ratings.some(rating => rating < 60)).map(emp => emp.name);
const averages = employees.map(emp => {
    const avg = emp.ratings.reduce((sum, rating) => sum + rating, 0)/emp.ratings.length;
    return { ...emp, average: avg };
});

const topperf = averages
    .filter(emp => emp.average > 85)
    .sort((a, b) => b.average - a.average)
    .map(emp => `${emp.name} : ${emp.average.toFixed(2)}`); 
